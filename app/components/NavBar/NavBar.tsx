  ...
  return (
    <div className="nav-bar-wrapper w-full">
      ...
      <nav className="desktop-navbar mx-auto max-w-screen-lg w-full px-14 lg:px-0 py-10 flex flex-row items-center justify-between text-navBar-link text-xl grid-flow-col">
        <div className="nav-logo text-3xl font-medium uppercase text-navBar-linkHover hover:text-navBar-link underlined">
          <a href="/">Alissa N</a>
        </div>
        {topLevelLinks.map((link) => (
          <a
            className="nav-link dark underlined hover:text-navBar-linkHover links-wrapper w-fit"
            href={link.href}
          >
            {link.displayName}
          </a>
